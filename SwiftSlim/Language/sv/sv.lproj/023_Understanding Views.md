...
När vi först mötte SwiftUI fanns det många nyckelord i `ContentView` som vi inte förstod.

Till exempel `struct`, `View`, `var`, `body`, `some View` och andra nyckelord.

Vid den tiden visste vi kanske bara detta: **om vi skriver kod inne i `body` visas innehållet på skärmen.**

Nu när vi har lärt oss `struct`, variabler, metoder och protokoll kan vi förstå den här koden igen och använda den för att förstå relationen mellan SwiftUI-views.

## Återbesöka ContentView

Efter att ha skapat ett SwiftUI-projekt genererar Xcode som standard en `ContentView`-fil.
...
Den här koden kan förstås genom att delas upp i flera delar.
...
Den här raden importerar SwiftUI-frameworket.

Efter att ha importerat SwiftUI kan vi använda SwiftUI-views som `View`, `Text`, `Image`, `VStack` och `Button`.

## ContentView-strukturen

Den här kodraden skapar en struktur:
...
betyder att `ContentView` är en struktur.

I förra lektionen lärde vi oss om `struct`. Den kan gruppera relaterade fält och innehålla egenskaper och metoder.
...
I det här exemplet innehåller `ContentView` tre saker: `name`, `printName` och `body`.

Med andra ord kan `struct` gruppera relaterade egenskaper, metoder och view-innehåll.

![contentView](../../Resource/023_contentVIew.png)

## View är ett protokoll

Fortsätt titta på den här kodraden:

```swift
struct ContentView: View 
```

`View` efter kolon betyder att `ContentView` följer protokollet `View`.

Du kan förstå det så här: om `ContentView` vill bli en SwiftUI-view måste den uppfylla kraven i protokollet `View`.

### Protokollet Identifiable
...
### Vad händer om vi inte följer View?
...
## body är startpunkten för att visa en view
...
### Namnet body kan inte ändras fritt
...
**some View förenklar returtypen**
...
### some View kan inte returnera vilken view som helst fritt
...
### Varför kan body direkt innehålla flera views?
...
### Dela upp den övre viewn

For example, split out the top area:

```swift
var topHem: some View {
    HStack {
        Text("Hem")
            .font(.largeTitle)
        Spacer()
        Image(systemNamn: "bell")
    }
}
```

Here, `topHem` is a variable that returns a view.
...
### some View returnerar en enda view
...
In this code, `topHem` returns either `HStack` or `Text` depending on the content of `step`, which are two different view types.
...
### Varför uppdelade views inte använder parenteser
...
**Egenskaper behöver inte parenteser.**
...
### När man använder view-egenskaper
