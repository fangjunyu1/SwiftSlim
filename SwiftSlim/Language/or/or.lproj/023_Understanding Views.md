...
ଏହି ପାଠରେ, ଆମେ ପ୍ରଥମ `ContentView` କୁ ପୁଣି ଫେରିବୁ।
...
ଆମେ ପ୍ରଥମେ SwiftUI ସହିତ ପରିଚିତ ହେବାବେଳେ, `ContentView` ଭିତରେ ଆମେ ନ ବୁଝିଥିବା ଅନେକ keyword ଥିଲା।

ଉଦାହରଣ ସ୍ୱରୂପ `struct`, `View`, `var`, `body`, `some View` ଇତ୍ୟାଦି keyword।

ସେ ସମୟରେ ଆମେ ବୋଧହୁଏ କେବଳ ଏତିକି ଜାଣୁଥିଲୁ: **`body` ଭିତରେ code ଲେଖିଲେ, screen ଉପରେ content ଦେଖାଯାଏ।**

ଏବେ ଆମେ `struct`, variables, methods, protocols ଶିଖିସାରିଛୁ, ତେଣୁ ଏହି code କୁ ପୁଣି ବୁଝି SwiftUI views ମଧ୍ୟର ସମ୍ପର୍କ ବୁଝିପାରୁ।

## ContentView କୁ ପୁଣି ଦେଖିବା

SwiftUI project ସୃଷ୍ଟି କରିବା ପରେ, Xcode default ଭାବରେ ଗୋଟିଏ `ContentView` file generate କରେ।
...
ଏହି code କୁ କିଛି ଭାଗରେ ଭାଙ୍ଗି ବୁଝିପାରିବ।
...
ଏହି line SwiftUI framework କୁ import କରେ।

SwiftUI import କରିବା ପରେ, ଆମେ `View`, `Text`, `Image`, `VStack`, `Button` ପରି SwiftUI views ବ୍ୟବହାର କରିପାରୁ।

## ContentView structure

ଏହି code line ଗୋଟିଏ structure ସୃଷ୍ଟି କରେ:
...
ମାନେ `ContentView` ଗୋଟିଏ structure ଅଟେ।

ପୂର୍ବ ପାଠରେ ଆମେ `struct` ବିଷୟରେ ଶିଖିଥିଲୁ। ଏହା ସମ୍ପର୍କିତ fields କୁ ଏକଠା କରିପାରେ, ଏବଂ properties, methods ରଖିପାରେ।
...
ଏହି example ରେ, `ContentView` ଭିତରେ ତିନୋଟି ଜିନିଷ ଅଛି: `name`, `printName`, ଏବଂ `body`।

ଅନ୍ୟ ଭାବେ କହିଲେ, `struct` ସମ୍ପର୍କିତ properties, methods, view content କୁ ଏକଠା କରିପାରେ।

![contentView](../../Resource/023_contentVIew.png)

## View ଗୋଟିଏ protocol

ଏହି code line କୁ ଆଗକୁ ଦେଖନ୍ତୁ:

```swift
struct ContentView: View 
```

colon ପରେ ଥିବା `View` ମାନେ `ContentView` `View` protocol କୁ conform କରେ।

ଏହାକୁ ଏଭଳି ବୁଝିପାରିବେ: `ContentView` SwiftUI view ହେବାକୁ ଚାହିଲେ, `View` protocol ର requirements ପୂରଣ କରିବା ଦରକାର।
...
struct ContentView: View {
    var body: some View {
        VStack(spacing: 30) {
            ...
            Button("ମୋତେ କ୍ଲିକ୍ କରନ୍ତୁ") {
                print("ମୋତେ କ୍ଲିକ୍ କରନ୍ତୁ ")
            }
...
Here, `topହୋମ` is a variable that returns a view.
...
In this code, `topହୋମ` returns either `HStack` or `Text` depending on the content of `step`, which are two different view types.
