...
## Ponownie przyjrzyjmy się ContentView
...
### Co się stanie, jeśli nie zastosujemy się do View?
...
## body jest punktem wejścia do wyświetlania widoku
...
### Nazwy body nie można dowolnie zmieniać
...
**Jeszcze bardziej złożone typy widoków**
...
**some View upraszcza typ zwracany**
...
### some View nie może dowolnie zwracać dowolnego widoku
...
### Dlaczego body może bezpośrednio zawierać wiele widoków?
...
### Wydzielenie górnego widoku

For example, split out the top area:

```swift
var topStrona główna: some View {
    HStack {
        Text("Strona główna")
            .font(.largeTitle)
        Spacer()
        Image(systemImię: "bell")
    }
}
```

Here, `topStrona główna` is a variable that returns a view.
...
### if zwracający wiele widoków

For example:

```swift
var topStrona główna: some View {
    if step == "Strona główna" {
        HStack {
            Text("Strona główna")
                .font(.largeTitle)
            Spacer()
...
In this code, `topStrona główna` returns either `HStack` or `Text` depending on the content of `step`, which are two different view types.
...
### Dlaczego wydzielone widoki nie używają nawiasów
...
**Właściwości nie potrzebują nawiasów.**
...
## Właściwość widoku czy nowy widok?
...
### Kiedy używać właściwości widoku
