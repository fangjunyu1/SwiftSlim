...
У цьому уроці ми повертаємося до початкового `ContentView`.
...
Коли ми вперше зіткнулися зі SwiftUI, у `ContentView` було багато ключових слів, яких ми не розуміли.

Наприклад, `struct`, `View`, `var`, `body`, `some View` та інші ключові слова.

Тоді ми, можливо, знали лише це: **якщо написати код всередині `body`, вміст з’явиться на екрані.**

Тепер, коли ми вивчили `struct`, змінні, методи та протоколи, можемо знову зрозуміти цей код і використати його, щоб зрозуміти зв’язок між SwiftUI views.

## Знову розглядаємо ContentView

Після створення SwiftUI-проєкту Xcode за замовчуванням генерує файл `ContentView`.
...
Цей код можна зрозуміти, розділивши його на кілька частин.
...
Цей рядок імпортує framework SwiftUI.

Після імпорту SwiftUI ми можемо використовувати SwiftUI views, такі як `View`, `Text`, `Image`, `VStack` і `Button`.

## Структура ContentView

Цей рядок коду створює структуру:
...
означає, що `ContentView` є структурою.

У попередньому уроці ми вивчили `struct`. Він може групувати пов’язані поля разом і містити properties та methods.
...
У цьому прикладі `ContentView` містить три речі: `name`, `printName` і `body`.

Іншими словами, `struct` може групувати пов’язані properties, methods і view content.

![contentView](../../Resource/023_contentVIew.png)

## View — це protocol

Продовжимо дивитися на цей рядок коду:

```swift
struct ContentView: View 
```

`View` після двокрапки означає, що `ContentView` відповідає protocol `View`.

Це можна зрозуміти так: якщо `ContentView` хоче стати SwiftUI view, він має виконати requirements protocol `View`.
...
### Що станеться, якщо не відповідати View?
...
## body — точка входу для відображення view
...
### Назву body не можна змінювати довільно
...
**Проблема невідповідності типів**
...
### some View не може довільно return будь-який view
...
### Чому body може напряму містити кілька views?
...
**Спочатку використовуйте containers**
...
struct ContentView: View {
    var body: some View {
        VStack(spacing: 30) {
            VStack(spacing: 20) {
                Text("Ласкаво просимо до Qinote")
                Text("Доброго ранку")
            }
            HStack {
                Color.blue
...
### Розділення верхнього view

For example, split out the top area:

```swift
var topГоловна: some View {
    HStack {
        Text("Головна")
            .font(.largeTitle)
        Spacer()
        Image(systemІм’я: "bell")
    }
}
```

Here, `topГоловна` is a variable that returns a view.
...
In this code, `topГоловна` returns either `HStack` or `Text` depending on the content of `step`, which are two different view types.
...
### Чому розділені views не використовують дужки
...
**Properties не потребують дужок.**
...
### Коли використовувати view properties
