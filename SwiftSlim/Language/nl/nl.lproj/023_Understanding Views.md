...
Toen we SwiftUI voor het eerst tegenkwamen, stonden er in `ContentView` veel trefwoorden die we nog niet begrepen.

Bijvoorbeeld `struct`, `View`, `var`, `body`, `some View` en andere trefwoorden.

Op dat moment wisten we misschien alleen dit: **als we code in `body` schrijven, verschijnt de inhoud op het scherm.**

Nu we `struct`, variabelen, methoden en protocollen hebben geleerd, kunnen we deze code opnieuw begrijpen en daarmee de relatie tussen SwiftUI-views begrijpen.

## ContentView opnieuw bekijken

Na het maken van een SwiftUI-project genereert Xcode standaard een `ContentView`-bestand.
...
Deze code kun je begrijpen door hem in meerdere delen op te splitsen.
...
Deze regel importeert het SwiftUI-framework.

Na het importeren van SwiftUI kunnen we SwiftUI-views zoals `View`, `Text`, `Image`, `VStack` en `Button` gebruiken.

## De ContentView-structuur

Deze coderegel maakt een structuur:
...
betekent dat `ContentView` een structuur is.

In de vorige les hebben we `struct` geleerd. Het kan gerelateerde velden groeperen en eigenschappen en methoden bevatten.
...
In dit voorbeeld bevat `ContentView` drie dingen: `name`, `printName` en `body`.

Met andere woorden, `struct` kan gerelateerde eigenschappen, methoden en view-inhoud samen groeperen.

![contentView](../../Resource/023_contentVIew.png)

## View is een protocol

Kijk verder naar deze coderegel:

```swift
struct ContentView: View 
```

De `View` na de dubbele punt betekent dat `ContentView` voldoet aan het `View`-protocol.

Je kunt het zo begrijpen: als `ContentView` een SwiftUI-view wil worden, moet het voldoen aan de eisen van het `View`-protocol.

### Het Identifiable-protocol

Toen we eerder de filmlijst leerden, kwamen we het `Identifiable`-protocol tegen.
...
Het `Identifiable`-protocol vereist: **dit type moet een `id`-veld hebben dat zichzelf identificeert.**
...
Het `View`-protocol werkt op dezelfde manier.

Als een type een SwiftUI-view wil worden, moet het voldoen aan het `View`-protocol.

De belangrijkste eis van het `View`-protocol is het leveren van een `body`.
...
### Wat gebeurt er als we View niet volgen??
...
Dit is slechts een gewone structuur, geen SwiftUI-view.
...
## body is het startpunt voor het weergeven van een view

In SwiftUI is `body` het startpunt van de inhoud van een view.
...
Dit kan in drie delen worden opgesplitst:
...
Dit definieert een variabele met de naam `body`.
