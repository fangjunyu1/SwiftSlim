...
我还记得第一次安装 App 到 iPhone 上时，我非常高兴，因为我可以更直观的看到我的开发成果。这是一种正向反馈，当我们可以使用自己开发的 App 时，兴趣就会推动我们自发的学习更多知识。
...
这通常不是程序逻辑错误，而是因为调试进程被外部终止。属于可忽略级别的提示。

模拟器相较于 Xcode 预览更接近真实运行环境，但仍然不是完整设备环境。它适用于布局验证、多机型适配测试。
