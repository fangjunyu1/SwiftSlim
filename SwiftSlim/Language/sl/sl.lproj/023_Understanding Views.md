...
Ko smo se prvič srečali s SwiftUI, je bilo v `ContentView` veliko ključnih besed, ki jih nismo razumeli.

Na primer `struct`, `View`, `var`, `body`, `some View` in druge ključne besede.

Takrat smo morda vedeli samo to: **če kodo napišemo znotraj `body`, se vsebina prikaže na zaslonu.**

Zdaj, ko smo se naučili `struct`, spremenljivke, metode in protokole, lahko to kodo znova razumemo in jo uporabimo za razumevanje odnosa med SwiftUI views.

## Ponovni pogled na ContentView

Po ustvarjanju SwiftUI projekta Xcode privzeto ustvari datoteko `ContentView`.
...
To kodo lahko razumemo tako, da jo razdelimo na več delov.
...
Ta vrstica uvozi framework SwiftUI.

Po uvozu SwiftUI lahko uporabljamo SwiftUI views, kot so `View`, `Text`, `Image`, `VStack` in `Button`.

## Struktura ContentView

Ta vrstica kode ustvari strukturo:
...
pomeni, da je `ContentView` struktura.

V prejšnji lekciji smo spoznali `struct`. Lahko združuje povezana polja ter vsebuje lastnosti in metode.
...
V tem primeru `ContentView` vsebuje tri stvari: `name`, `printName` in `body`.

Z drugimi besedami, `struct` lahko združi povezane lastnosti, metode in vsebino view.

![contentView](../../Resource/023_contentVIew.png)

## View je protokol

Nadaljujmo s pogledom na to vrstico kode:

```swift
struct ContentView: View 
```

`View` za dvopičjem pomeni, da `ContentView` ustreza protokolu `View`.

To lahko razumeš tako: če želi `ContentView` postati SwiftUI view, mora izpolniti zahteve protokola `View`.
...
### Kaj se zgodi, če ne upoštevamo View?
...
## body je vstopna točka za prikaz view
...
### Imena body ni mogoče poljubno spreminjati
...
**some View poenostavi vrnjeni tip**
...
### some View ne more poljubno vrniti kateregakoli view
...
### Zakaj lahko body neposredno vsebuje več views?
...
### Zakaj razdeljeni views ne uporabljajo oklepajev
...
**Lastnosti ne potrebujejo oklepajev.**
...
### Kdaj uporabiti view properties
